...
import태그
	...
	먼저 포함시킨후 컴파일하는 형식이 아니라, include 액션태그와 같이
	컴파일이 먼저 진행된 후 결과를 포함시키게된다. 
...
-->
    ...
    <!--  
    var속성을 
    	미사용시 : include 액션태그와 동일하게 현재 위치에 즉시 외부문서를
    		포함시킨다.
    ...
    만약 절대경로를 사용해야 한다면 하드코딩 하는것보다 request내장객체에서
    제공하는 메서드를 사용하는것이 좋다. 
    웹프로그래밍은 웹서버에 배포하는것이 목적이므로 서버의 환경이 달라지면
    경로까지 수정해야하는 경우 이를 최소화할수 있도록 개발하는것이 좋다.
    ...
    <!-- 앞에서 선언한 import 태그의 var 속성의 변수를 통해 문서의 
    내용을 출력한다.  -->
    <h4>다른 문서 삽입하기</h4>
    ${contents }
