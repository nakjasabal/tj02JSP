...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!--  
JSTL의 core태그를 사용하기 위한 지시어 선언. prefix는 접두어, uri는 태그를 
지원하는 예약어가 들어간다. 
-->    
...
<!--  
set태그
	: 변수를 선언할때 사용. JSP의 setAttribute()와 동일한 기능을 한다. 
...
-->
	...
	<!-- value에는 일반값, EL, 표현식을 모두 사용할 수 있다. 또한 태그 
	사이에 value를 삽입할 수도 있다.  -->
    <c:set var="directVar" value="100" />
    <c:set var="elVar" value="${ directVar mod 5}" />
    <c:set var="expVar" value="<%= new Date() %>" />
    <c:set var="betweenVar">변수값 요렇게 설정</c:set>

	<!-- 속성명이 중복되지 않는다면 영역을 표시하는 내장객체를 생략할 수
	있다.  -->
    <h4>EL을 이용해 변수 출력</h4>
    <ul>
    ...
    <!-- 빈 객체를 먼저 생성한 후 target, property를 통해 멤버변수의 값을
    설정한다. 이때 자바빈의 setter()가 사용된다. -->
